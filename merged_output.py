...
# ===== File: src/open_clip/.ipynb_checkpoints/__init__-checkpoint.py =====
...
# ===== File: src/open_clip/.ipynb_checkpoints/spaglam_model-checkpoint.py =====
...
# ===== File: src/open_clip_train/.ipynb_checkpoints/__init__-checkpoint.py =====


# ===== File: src/open_clip_train/.ipynb_checkpoints/data-checkpoint.py =====
...
# ... (在其他导入语句之后)
# --- 修改这里的导入 ---
from .spaglam_data import get_spaglam_dataset, DataInfo
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    return data
# ===== File: src/open_clip_train/.ipynb_checkpoints/main-checkpoint.py =====
...
# ===== File: src/open_clip_train/.ipynb_checkpoints/params-checkpoint.py =====
...
# ===== File: src/open_clip_train/.ipynb_checkpoints/spaglam_data-checkpoint.py =====
...
class SpaGLaMWDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self, tar_urls, anndata_path, image_processor, tokenizer):
        ...
        logging.info(f"Loading AnnData master file from {anndata_path}...")
        self.adata = sc.read_h5ad(anndata_path, backed='r')
        logging.info("AnnData master file loaded successfully.")

    def _process_sample(self, sample):
        """
        核心map函数，处理从WebDataset流出的单个中心点样本。
        """
...
# src/open_clip_train/spaglam_data.py (修正后的版本)

# ... (顶部的 imports 和 DataInfo 类保持不变) ...
# ... (process_sota_sample 函数保持不变) ...

def get_spaglam_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    """
    SOTA SpaGLaM 数据集的工厂函数。
    此版本正确地应用了 .with_epoch() 方法。
    """
    input_shards = args.train_data if is_train else args.val_data
    assert input_shards is not None, "必须提供 WebDataset 路径 (--train-data / --val-data)。"

    bound_process_fn = lambda sample: process_sota_sample(sample, preprocess_fn, tokenizer)
    ...
    pipeline = [
        ...
        wds.map(bound_process_fn, handler=wds.warn_and_continue),
        wds.select(lambda x: x is not None),
        # 注意：为了清晰起见，我们将批处理移到了 with_epoch 之后，尽管两种方式都可以。
    ]

    # 第1步：从阶段列表中创建初始的 DataPipeline 对象
    dataset = wds.DataPipeline(*pipeline)

    # 第2步：如果是训练数据集，则应用长度限制
    num_samples = args.train_num_samples if is_train else args.val_num_samples
    if is_train and num_samples:
        # 计算每个 worker 应该处理的样本数量。
        # 这能确保在一个 epoch 内，所有 worker 处理的样本总数约等于 train_num_samples。
        # 这里的整除是正确的。
        num_worker_samples = num_samples // args.world_size // args.workers
        
        # 正确用法：在现有的 dataset 对象上调用 .with_epoch()
        dataset = dataset.with_epoch(num_worker_samples)
    
    # 第3步：对（可能已被限制长度的）流水线应用批处理
    dataset = dataset.batched(args.batch_size, partial=not is_train, collation_fn=PyGBatch.from_data_list)
    
    # 第4步：配置 WebLoader
    dataloader = wds.WebLoader(
        dataset,
        batch_size=None,  # 批处理已在流水线内部完成
        shuffle=False,
        num_workers=args.workers,
        pin_memory=True,
        persistent_workers=args.workers > 0,
    ...
    if num_samples is not None:
        ...
        if args.world_size > 0:
            ...
            dataloader.num_batches = num_samples // args.batch_size
    else:
        dataloader.num_samples = 0
        dataloader.num_batches = 0

    from open_clip_train.data import SharedEpoch
    return DataInfo(dataloader=dataloader, shared_epoch=SharedEpoch(epoch))
# ===== File: src/open_clip_train/.ipynb_checkpoints/train-checkpoint.py =====
