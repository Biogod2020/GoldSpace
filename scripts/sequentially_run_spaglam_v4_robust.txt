...
# ==============================================================================
# SpaGLaM SOTA 串行实验主管脚本 v4 (健壮版)
# - 支持 2-hop 子图, 多架构对比, 残差连接
# - 自动跳过已完成的实验
# - 自动从断点恢复中断的训练
# ==============================================================================
# 用法:
# 1. 确认下面的路径和参数配置正确。
# 2. 运行此脚本: `bash sequentially_run_spaglam_v4_robust.sh`
...
# 【修改 1】: 将 epochs 定义为变量，作为唯一的真实来源
TOTAL_EPOCHS=12
# 实验日志总根目录 (使用与上次相同的根目录，以便能够检测到已完成的实验)
# 如果你想强制重跑所有实验，只需修改这个目录名即可
LOGS_BASE_DIR="/cpfs01/projects-HDD/cfff-afe2df89e32e_HDD/jjh_19301050235/spaglam_train/train_log/SpaGLaM-Runs-2Hops-20250807-113725"
mkdir -p ${LOGS_BASE_DIR} # 确保根目录存在
...
TEST_DATA_URL="/cpfs01/projects-HDD/cfff-afe2df89e32e_HDD/jjh_19301050235/my_data/spaglam_precom_subgraphs/shard-{000090..000095}.tar"
TEST_NUM_SAMPLES=200000

# --- 3. 通用训练参数 ---
# 【修改 2】: 增加了 --resume latest 来激活内部恢复机制
COMMON_ARGS=" \
  --model               ViT-B-32 \
...
for experiment in "${EXPERIMENTS[@]}"; do
    ...
    # 【修改 3】: 检查最终检查点文件是否存在
    FINAL_CKPT_PATH="${LOGS_BASE_DIR}/${EXP_NAME}/checkpoints/epoch_${TOTAL_EPOCHS}.pt"
    
    if [ -f "$FINAL_CKPT_PATH" ]; then
        echo "✅  实验 '${EXP_NAME}' 已完成。找到最终检查点: ${FINAL_CKPT_PATH}"
        echo "⏭️  跳过训练阶段。"
        continue # 直接跳到下一个循环
    fi

    echo "🚀  开始或恢复训练实验: ${EXP_NAME}"
    ...
    # 如果实验中断，下次运行时 --resume latest 会自动生效
...
for experiment in "${EXPERIMENTS[@]}"; do
    ...
    MODEL_CKPT="${LOGS_BASE_DIR}/${EXP_NAME}/checkpoints/epoch_${TOTAL_EPOCHS}.pt"
